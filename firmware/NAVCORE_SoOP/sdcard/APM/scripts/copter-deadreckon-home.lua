-- Vendored from ArduPilot tag Copter-4.7.0 (the same tag tools/build_firmware.sh
-- pins the firmware tree to): libraries/AP_Scripting/applets/copter-deadreckon-home.lua
-- Install: copy to the microSD at APM/scripts/.
--
-- THIS FILE CARRIES A LOCAL PATCH. It is NOT byte-identical to upstream, and an earlier
-- version of this header wrongly said it was. Three changes, all in the GPS-bad timer:
--
--   1. `now_ms - gps_bad_start_time_ms > gps_trigger_sec:get()`
--      became          `... > (gps_trigger_sec:get() * 1000)`
--      UPSTREAM UNIT BUG. now_ms is millis(); DR_GPS_TRIGG_SEC is documented in SECONDS
--      and defaults to 3. Upstream compares milliseconds against seconds, so the timer
--      expires after 3 ms instead of 3 s and the GPS-bad debounce does nothing. The same
--      script's other elapsed-time test uses `recovery_delay_ms = 3000`, which is the
--      convention this line breaks. Worth reporting upstream.
--   2. gps_bad_start_time_ms is reset to 0 when GPS recovers, so a later degradation
--      starts a fresh count instead of measuring from the first-ever bad sample.
--   3. gps_bad_start_time_ms is reset to 0 in the `else` branch when GPS is good, for
--      the same reason. Without 2 and 3 the latched start time means any single bad
--      sample much later trips the timer instantly - which only becomes reachable once
--      fix 1 makes the debounce real.
--   4. add_param default for DR_ENABLE_DIST changed 50 -> 30
--   5. DR_NEXT_MODE stays at the applet's own default of 6 (RTL). This board previously
--      specified -1 ("return to the mode in use before dead reckoning") and argued for it
--      at length. THE ARGUMENT WAS WRONG, and it was never tested because the value was
--      being silently discarded (see below) - the aircraft had been flying 6 all along.
--
--      With -1 actually applied, measured over 8 runs of soop_dropout against 8 runs on 6:
--
...
--
--      -1 returns the aircraft to whatever was flying it. If that was a pilot in Loiter,
--      fine. If it was an autonomous mission - and this aircraft's whole purpose is
--      autonomous flight - then the applet hands control straight back to the thing that
--      was flying it away, which re-triggers seconds later. One run oscillated through
--      13 mode changes. Going home and STAYING there is the correct response to losing
--      the position source; that is what 6 does.
--
-- WHY 4 AND 5 ARE IN THIS FILE AND NOT IN defaults.parm - THEY WERE, AND IT DID NOTHING.
--
-- These parameters do not exist until THIS SCRIPT creates them. ArduPilot parses the
-- ROMFS/boot defaults file before the scripting engine runs, so every DR_* line in
-- defaults.parm names a parameter that does not exist yet and is silently discarded.
-- Seven of the nine happened to match the applet's own defaults and so looked fine;
-- these two did not, and the aircraft flew values it was never configured with:
--
--     DR_ENABLE_DIST   defaults.parm said 30, the applet actually used 50
--     DR_NEXT_MODE     defaults.parm said -1, the applet actually used  6 (plain RTL)
--
-- DR_ENABLE_DIST is the one that mattered. The applet will not arm until the aircraft
-- has been that far from home with a good fix. At 50 m it frequently never arms at all,
-- so the aircraft's entire GPS-loss mitigation sat inert and the SITL divergence rate
-- was really "did it happen to wander past 50 m before it got lost". Setting the default
-- HERE makes it take effect on first boot with no ground-station step and no ordering
-- dependency. `./sitl/run_scenarios.sh --params` is the check that proves it.
--
-- Re-diff against upstream on every ArduPilot bump: if the bug is fixed upstream, drop
-- these and take theirs. Verify with:
--   diff -w -B <(grep -vE '^\s*--|^\s*$' $AP/libraries/AP_Scripting/applets/copter-deadreckon-home.lua) \
--              <(grep -vE '^\s*--|^\s*$' firmware/NAVCORE_SoOP/sdcard/APM/scripts/copter-deadreckon-home.lua)
--
-- -w -B MATTERS. This copy was reindented when it was vendored - trailing spaces
-- stripped, aligned end-of-line comments collapsed to one space - so a plain diff
-- returns about 120 lines of whitespace noise with the three real changes buried in it,
-- and the reader gives up exactly when the check is worth most. With -w -B it prints
-- those three hunks and nothing else. Verified 2026-09-02 against Copter-4.7.0: the
-- output is exactly the three changes listed above.
--
...
assert(param:add_param(PARAM_TABLE_KEY, 2, 'ENABLE_DIST', 30), 'could not add DR_ENABLE_DIST param') -- distance from home (in meters) beyond which the dead reckoning will be enabled
...
function update()
  ...
  if gps_bad then
    ...
    if (gps_speed_acc_bad or gps_num_sat_bad) then
      ...
      elseif (now_ms - gps_bad_start_time_ms > (gps_trigger_sec:get() * 1000)) then
        gps_bad = true
      end
    else
      gps_bad_start_time_ms = 0
