...
export doesn't contain six copper layers. `fab/ORDER.md` has the exact JLCPCB settings,
...
| `docs/navcore-runbook.tex` / `.pdf` | the order-to-first-flight runbook, 17 pages |
...
The written docs are held back until they are polished enough to publish; they are on disk
and the tooling reads them. What each one answers:

| question | document |
|---|---|
| What do I buy, from where, and what must I check first? | **`docs/BUYING.md`** — order sheet, blockers, pre-order checks |
...
| What is still open, and why was a superseded decision made that way? | **`docs/KNOWN-ISSUES.md`** (current) / **`docs/HISTORY.md`** (nothing there is current) |
| Board state, design rules, remaining connections | **`docs/LAYOUT.md`** / **`docs/ROUTING-TODO.md`** (generated) |
| The order-to-flight runbook, compiled | **`docs/navcore-runbook.pdf`** |
