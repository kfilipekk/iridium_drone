...
check_design.py proves the netlist is valid. check_hwdef.py proves the firmware does not
name pins the board lacks. Both passed on a board where the rangefinder was held in
shutdown by its own firmware and the ESC telemetry line was soldered to a transmit pin,
because neither of them compares DIRECTION.

Four things are checked, against design.PIN_INTENT:

  contention   the hwdef configures a pin as a push-pull OUTPUT while the board has a
               device output on the same net - two drivers on one node
  boot level   the hwdef's reset level is not the level the device needs to operate
  role         the pin's peripheral function runs the wrong way: a UART TX pin on a
               signal that arrives, or an RX pin on one that leaves
  pull fight   the hwdef drives a pin against a fitted pull resistor

Plus a coverage check: any net joining an MCU pin to a real device must declare its
intent, so the table cannot quietly fall behind the netlist.

Exit code is non-zero if anything in the first three categories fires. A pull fight is
reported as a warning - it is sometimes deliberate.
...
def pulls():
    ...
    Derived from the netlist rather than declared, so a pull that is added or removed
    is picked up without anyone remembering to update a table.
    """
    where = {}
    for net, specs in design.NETS.items():
        for sp in specs:
...
def main():
    ...
    for pin in sorted(pins):
        ...
        # 3. role - a one-way peripheral pointed the wrong way
