...
WHY THIS EXISTS. design.PLATES and design.FRAME_CAD cite "ezdxf flatten of
So1-V6-7inDC-2025-JUL-07.dxf, 2026-09-04" fourteen times, but the script that did it
was never committed, the DXF was never on disk, and ezdxf was installed nowhere. Every
mechanical claim downstream (standoff length, ground clearance, the 35 mm standoff
purchase) rested on numbers nobody could re-derive. Same defect shape as the JLC
mirror and the .libraries directory: an authority that lived outside the repo.

The DXF is 72 MB, so the FILE stays in ~/.cache/navcore/frame/ and the PARSE is what
gets versioned: cad/frame-dxf.json, keyed to the DXF's sha256 so a re-export by TBS
cannot be mistaken for the one this design was checked against.

What it extracts, and what each is good for:
  plates      outline W x L per plate and every hole (dia, x, y) relative to the
              plate's bbox centre - the stack patterns, and the top plate's own hole
              pattern (which is NOT the 30.5 stack; see the report this produced)
  arms        the motor-end slots: their radius from the arm's motor centre, which
              is the number that decides whether a 19x19 motor bolts on at all
  labels      the drawing's own thickness labels, mapped to the nearest outline
  hardware    the frame's BOM text as drawn (M3x10 = 16pcs, standoffs 30/22 ...)
...
# The plates, identified by the layer TBS drew them on and their outline size. Names
# are OURS (the drawing calls them Draw3/13/14); the thickness labels below say which
# is which by position, and TBS's product page gives the same three thicknesses.
