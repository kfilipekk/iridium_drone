...
def resolve(pins, spec):
    ...
    Name matching tolerates ST's alternate-function suffixes: the symbol names pin 26
    "PC14-OSC32_IN" while design.py writes "PC14". A prefix is accepted only when it is
    UNIQUE, so an ambiguous name is reported rather than guessed at. This lives here,
    rather than being reimplemented by each caller, because two copies of a resolver
    drift and the copy that drifts is the one that says everything is fine.
    """
