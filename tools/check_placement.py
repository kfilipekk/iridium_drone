...
The first layout routed to 61% and stalled. The cause was not the router - it was a
placement that optimised for wasted area instead of electrical distance, leaving VCAP2
30 mm from its pin. This is the acceptance test for the fix.
"""
import sys, os, math
...
def main():
    ...
    # An MCU-to-edge-pad net cannot be short: the MCU is central and the pad must
    # reach an edge. Judge span as a fraction of the board diagonal instead of an
    # absolute figure, which is what makes it comparable across board sizes.
...
# The stack screws land on the board, not just through it. Measured against pad
# copper rather than the footprint bounding box - the bbox includes silkscreen and
# courtyard graphics, which are not physical and give a false alarm of ~0.5 mm.
# Only a metal screw head bearing directly on copper is a hard failure. The silicone
# grommet flange and a washer land on solder mask, and the pads under them are mask
# covered, so a fraction of a millimetre of overlap there is an assembly note, not a
# defect - calling it a failure would make this gate cry wolf.
