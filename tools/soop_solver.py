...
WHAT THIS ANSWERS. The project rests on a claim it has never checked for itself: that
the Doppler shift on Iridium downlinks constrains receiver position well enough to
navigate. Every accuracy figure in this repo so far - "100-200 m", "180 m" - is quoted
from the literature. That is a reasonable starting point and it is not the same as
knowing what THIS geometry gives, how many bursts it needs, or how the error scales
with frequency-measurement noise.

So this implements the solve and then recovers a KNOWN position from synthetic
observations. If it cannot do that on clean geometry, nothing downstream matters.

THE MEASUREMENT MODEL. A receiver at rest sees a satellite transmitting at f_tx with
a shift set by range rate, plus its own oscillator error:

    df = f_tx * (-rdot/c + delta)          rdot = (v_sat - v_rx) . u
                                           u    = (r_sat - r_rx) / |r_sat - r_rx|

Four unknowns: receiver position (3) and the receiver's clock offset (1). The clock is
solved for rather than calibrated away, exactly as GPS solves for clock bias - which is
why the +-2.5 ppm TCXO is adequate and the +-30 ppm crystal it replaced was not: what
matters is that the offset is STABLE across the observation window, not that it is
small.

THE CLOCK UNKNOWN IS CARRIED IN HZ, NOT AS A FRACTION, and that is not cosmetic. With
delta dimensionless its Jacobian column is f_tx = 1.6e9 while the position columns are
around 1e-4, so the normal matrix spanned 23 orders of magnitude and the position
parameters were numerically invisible next to the clock. Measured before the rescale:
a noise-free solve was exact, 1 Hz of noise gave 47 km. Carrying the clock in Hz makes
that column 1.0 and the system ordinarily conditioned.

Jacobian, derived rather than differenced:

    d(df)/d(r_rx)     = (f_tx/c) * (v_sat - v_rx)^T (I - u u^T) / rho
    d(df)/d(clock_hz) = 1

WHAT THIS IS NOT. The orbit model here is a circular propagator at Iridium's altitude
and inclination, not SGP4. That is deliberate and it is enough for this question: the
solver consumes satellite POSITION and VELOCITY, so where they come from does not
change whether the geometry inverts. Production needs SGP4 and TLE ephemeris, and the
ephemeris error then enters the error budget directly - see the note at the bottom.

Nor does it say anything about whether the receiver can HEAR a burst, or measure its
frequency to the precision assumed here. That is an RF question and it needs the
antenna, the SAWbird+ and a sky view.

    python3 tools/soop_solver.py            # self-test
    python3 tools/soop_solver.py --sweep    # accuracy vs noise and burst count
"""
import math
import random
import sys
...
# ----------------------------------------------------------------- linear algebra
def solve4(A, b):
    """Gauss-Jordan with partial pivoting on a small dense system. Returns None if
    singular - a degenerate geometry must report failure, not a plausible number."""
    n = len(b)
...
def circular_orbit(t, raan, arg0, alt=IRIDIUM_ALT, inc=IRIDIUM_INC):
    """Position and velocity of a satellite on a circular orbit, in an inertial frame.
    Good enough to invert the geometry; NOT an ephemeris."""
    r = R_EARTH + alt
    n = math.sqrt(MU / r ** 3)
    u = arg0 + n * t
...
def predict_df(rs, vs, rr, clock_hz, f_tx=F_IRIDIUM):
    """Observed frequency offset from nominal. clock_hz is the receiver's oscillator
    error expressed at the carrier, in Hz - see the header on why not a fraction."""
...
def solve(obs, guess, f_tx=F_IRIDIUM, iters=40, tol=1e-4):
    ...
    Gauss-Newton. Returns (state, residual_rms_hz, n_iter) or (None, None, n) if the
    geometry is degenerate or it fails to converge."""
    x = list(guess)
...
def constellation(t, n_planes=6, per_plane=11):
    """Iridium-like: 6 planes, 11 satellites each, near-polar. Positions only - this is
    a GEOMETRY generator, not an ephemeris. See the header."""
    out = []
...
def make_pass(rr_true, clock_hz, n_obs, sigma_hz, seed, span_s=600.0):
    ...
    The first version asked for 40 and kept 5, because it generated random orbits and
    then discarded whatever was below the horizon. Four unknowns against five
    observations is barely determined, and that alone looked like noise sensitivity."""
