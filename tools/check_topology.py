...
def main():
    ...
    # ------------------------------------------------------------------
    # REVERSE-POLARITY P-FET: the gate must be defined, or the board is dead
    # with a CORRECT battery connected. Same class of check as the catch diode -
    # the fault is an ABSENT part, so it is keyed on the part number.
    #
    # The circuit is ON Semi AND90146/D Figure 4. The battery connects to the DRAIN,
    # the load to the SOURCE: the body diode conducts until the channel turns on, and
    # a reversed pack reverse-biases the diode so the rail stays dead. R46 pulls the
    # gate to GND (a P-FET conducts with Vgs < -VT, so gate-at-GND against a source
    # at VBAT fully enhances it); DZ1 clamps Vgs at ~15 V against the WST4041's
    # +-20 V rating. ORIENTATION IS THE CIRCUIT - source-to-battery would conduct a
    # reversed pack straight into the rail, which is the exact failure mode this
    # repo shipped for weeks with no protection at all.
    ...
    # DO NOT skip when the ref is absent. Every other rule in this file is keyed on a
    # part that something else depends on - delete U8 and the +5V rail vanishes, so a
    # dozen checks shout. NOTHING downstream depends on Q4: the rail works fine without
    # it, right up until a pack goes on backwards. So an absent Q4 was a SILENT PASS -
    # this whole section printed its header and nothing else, and the board could have
    # been ordered with no reverse-polarity protection and a green gate. That is the
    # same shape of hole as the missing catch diode, in the check written to close it.
    for ref, part in (("Q4", "WST4041"),):
        ...
        # 3. the orientation - drain to the BATTERY side, source to the load. This
        #    is the part of the circuit that defeats a reversed pack; written the
        #    other way round the body diode conducts it straight into the rail.
