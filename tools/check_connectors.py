...
Both J1 (USB-C) and J2 (ESC) shipped in the layout fitted backwards, mouths pointing into
the board. J1 was fatal - no cable could be plugged in, so the board could not be flashed
or talked to at all. Krystian caught it by eye on a 3D render.

Every automated check had passed them, and that is the point of this file. check_design,
check_hwdef, check_electrical, check_traces, check_placement, check_build and DRC all
validate ELECTRICAL correctness. Which way a connector faces is mechanical intent, and
nothing in the toolchain expressed it. check_build even measured J1's 0.83 mm to the board
edge - and passed it - without ever asking which way it faced.

So this checks three things, in order of how badly each one bites:

  1. The mating face DERIVED from the footprint agrees with design.MATING_FACE. Catches a
     footprint swap silently invalidating the declared value.
  2. The mouth points off the board. This is the J1 failure.
  3. There is straight, clear space in front of it for the plug. This is the J2 failure,
     and the reason 2 alone is not enough: J2's mouth sat 0.82 mm from the board edge and
     passed every clearance rule in the project while facing inboard into U1, 3.28 mm
     away - room for neither the plug body nor the fingers to fit and remove it.
...
# Hard failure below HALF the connector's own nominal allowance, rather than at some
# flat distance. A flat 2.5 mm let J2 through as a mere warning at 2.50 mm from the MCU,
# and J2 was not usable: a JST-SH plug carrying VBAT and four motor signals cannot be
# pushed in and pulled out of a 2.5 mm slot. Scaling to the declared allowance keeps the
# ratio honest across a USB-C overmould and a 1.0 mm-pitch JST alike.
...
def local_to_board(fp, lx, ly):
    ...
    Uses the same transform the surgery was verified with. Hand-rolled trigonometry got
    this wrong for bottom-side parts once already, so the flip is applied first, as a
    mirror in local x, and only then the rotation."""
    p = fp.GetPosition()
    px, py = p.x/1e6, p.y/1e6
    # A flipped footprint mirrors in local Y under this convention, not X. Checked
    # against J8's library file, whose 9 contacts are at local y -5.28: mirroring x
    # placed the microSD slot facing the TOP edge, mirroring y puts it at the bottom
    # where the card actually comes out. J1/J2/J3 never exposed this - none is flipped.
...
def derive_face(fp):
    ...
    Contacts leave a connector at the REAR, so the row holding most of them marks the back
    and the mouth is opposite. The courtyard should agree - it extends further past the
    front to reserve room for the plug - and that agreement is reported as the confidence.
    """
    rows = {}
    for pad in fp.Pads():
        p = pad.GetPosition()
...
def main():
    ...
    for ref, declared in sorted(design.MATING_FACE.items()):
        ...
        for other in board.GetFootprints():
            ...
            # Test pads and fiducials are bare copper with no body, so a plug passes
            # over them. Keyed on the FOOTPRINT rather than the reference prefix: the
            # LED strip's solder lands are PL1..PL3, which no prefix rule would catch,
            # and they are Connector:TestPoint like the rest. Everything else counts as
            # an obstruction - design.py carries no per-part heights, so this stays
            # deliberately conservative and will over-report rather than miss a J2.
            ...
            # Start just in FRONT of the mouth, not at it. At d = 0 the samples lie in
            # the connector's own face plane, where its neighbours legitimately sit
            # shoulder to shoulder - which flagged U11 as blocking J1 when U11 was
            # merely beside it and the plug approaches from off the board entirely.
            d = 0.3
            while d <= reach:
        ...
        if blockers:
            ...
            # Two thresholds, because they answer different questions and only one of
            # them is knowable from the board. CRITICAL is "nothing can mate here at
            # all" and is a hard failure - that is the J2 case, where U1 sat 3.28 mm in
            # front of the mouth. The nominal figure in design.MATING_CLEARANCE is a
            # plug-plus-bend allowance that was written down by hand, and failing a
            # board on a hand-picked constant is how this project talked itself into
            # inventing 0.15 mm clearances elsewhere. Below nominal is a warning naming
            # the measurement, for checking against the connector that actually arrives.
