...
J3 is the only real component on this board with no 3D model, and it is the tallest
top-side part - so every STEP and GLB export has been drawing a stack whose highest
point is missing. No computed check reads these models (heights come from
design.PART_HEIGHT), so this is about the exports being honest, not about a number.

There is no CAD kernel available here - no cadquery, no OCC, no FreeCAD, and OpenSCAD
cannot write STEP - so the B-rep is written out directly. A box is the one solid where
that is reasonable: 8 vertices, 12 edges, 6 planar faces, all of it mechanical.

The result is a BODY box, deliberately not a detailed connector. It carries the outline
and the height, which is all an export or a collision view needs from it, and claiming
more detail than that would be dishonest about where the numbers came from.
