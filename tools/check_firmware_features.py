...
WHY THIS EXISTS, and why check_params.py could not have caught it.

`TEMP1_TYPE 10` shipped in defaults.parm. `check_params.py` passed - the name is real,
the value 10 is a valid enum member, the firmware built, the board would boot and arm.
And the sensor was DEAD SILICON: `AP_TemperatureSensor::init` and `::update` had
compiled to empty stubs, and no backend object was linked at all.

The cause is a flash-budget default in AP_TemperatureSensor_config.h:

    HAL_PROGRAM_SIZE_LIMIT_KB <= 1024  ->  0, compiled out
    HAL_PROGRAM_SIZE_LIMIT_KB >  2048  ->  1, the real driver
    otherwise                          ->  2, "dummy methods for all vehicles
                                              except Sub and SITL"

This board is 2048 KB less a 128 KB bootloader reserve = 1920 KB, which lands in the
middle branch. ArduCopter got 2.

check_params.py validates names against `apm.pdef.xml`, which is generated from the
WHOLE SOURCE TREE. It therefore describes every parameter ArduPilot could have, not the
ones THIS board compiled - the repo's signature defect, a check measuring the thing
NEXT TO the property that matters.

So this reads the LINKED ELF. Two rules follow from how the bug hid:

  * SYMBOL PRESENCE IS NOT ENOUGH. "Enabled 2" emits real, correctly-named symbols that
    are two or four bytes of `bx lr`. Every assertion below carries a MINIMUM SIZE, and
    the stubs that started this failed it at 4 bytes against a floor of 16.
  * A MISSING ELF IS A FAILURE, NOT A SKIP. "Could not check" reported as "ok" is how
    the preflight footprint check once rendered SKIPPED as a pass.
