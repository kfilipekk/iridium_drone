...
WHY THIS EXISTS. `fab/NAVCORE-SoOP-order-bundle.zip` is the artefact that actually gets
uploaded - it is the whole point of `make_order_bundle.sh`. Every other fabrication check
in this repo looks at the loose files: `preflight.py` asserts the gerbers are newer than
the board file, and `check_cpl.py`/`check_design.py` work from `fab/*.csv`. NONE of them
opens the zip.

So the zip is a second copy of the fabrication output with nothing comparing it to the
first. Edit the board, regenerate the gerbers, forget to re-run the bundle script, and the
gate stays green while the thing you upload is the previous board. That is this project's
recurring defect exactly - a check measuring something adjacent to the property that
matters - and it lands on the one artefact where being wrong costs money rather than time.

WHAT IT ASSERTS:

  1. PRESENT   the bundle exists and carries everything the order needs: the BOM, the CPL,
               `gerbers.zip`, and the ordering instructions.
  2. IDENTICAL every BOM/CPL it carries is byte-identical to the one on disk, and every
               gerber inside `gerbers.zip` is byte-identical to `fab/gerbers/`.
  3. DERIVED   the placement counts quoted in HOW-TO-ORDER.txt equal the counts in the CPL
               that ships in the same bundle. The script derives them from that file at
               generation time precisely so they cannot be typed wrong; this re-derives
               them independently, so a hand-edited bundle is caught too.
    ...
    if "gerbers.zip" in names:
        ...
        for member in inner.namelist():
            ...
            if not os.path.exists(disk):
                ...
                ger_ok += 1
        # and nothing on disk was left out of it
