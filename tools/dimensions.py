...
Written because "does it fit" was being answered from a CAD model whose bottom-side part
height was wrong: check_build.py matched eight footprint patterns and silently assumed
1.0 mm for everything else, which covered J2 (a JST-SH is 2.9 mm), U1, both 10 uH power
inductors, every 1206 cap, the crystal and all the SOT-23s.

Anything marked [M] is measured from NAVCORE-SoOP.kicad_pcb and is exact. Anything else
carries its source. The two are kept apart on purpose: only one of them is a fact.
...
#
# These were six private copies, and two of them were the SUPERSEDED Mark4 frame:
# FRAME_H 35.0 mm and PLATE_T 3.0 mm from an AliExpress listing, against design.FRAME's
# inner_h 30.0 and bottom_t 2.5 for the TBS Source One this aircraft actually uses. One
# quantity, two files, two answers - the exact defect design.FRAME exists to end, and
# check_mechanical.py's header documents having removed the same copies from itself.
...
def main():
    ...
    # The bottom-side figure was a bare 2.5 constant here. It is now derived from the
    # board, and reported per variant because L5 - a 3.0 mm inductor on the underside -
    # is fitted only on the analogue-FPV build.
    ...
    # Nearest COPPER to a hole centre - pads, not footprint bounding boxes. A
    # footprint's bounding box includes its silkscreen reference text, and TP21's
    # sprawls right across a mounting hole; measuring that way reported 0.01 mm for a
    # pad that is actually 5.28 mm away.
    worst = None
    ...
    # This ladder started at the bottom plate and went straight to the ESC, omitting the
    # 6.0 mm arms and the 2.0 mm mid plate between them. That is 8.0 mm of frame, and it
    # is VERBATIM the bug check_mechanical.py:111-116 documents as fixed there - it
    # reported 7.7 mm of spare where there was -0.3 mm - left live in this sibling file.
    # The plate carrying the 30.5 mm stack pattern sits ABOVE the arm layer.
    z = PLATE_T[0]
    ...
    # Was a bare 2.5 with the source "L2/L5, 10 uH 1210 inductors" - L2 had moved to the
    # top side and the part is 3.0 mm, not 2.5. Derived now, worst case of the two builds.
    ...
    # NOT "a plate opening". A 30x30 stack does not sit inside a hole - it bolts to
    # four standoffs at 30.5 mm centres and OVERHANGS them by 7-8 mm a side, exactly as
    # the ESC does. What matters is whether a corner fouls an arm root or one of the
    # frame's own standoffs, at the board's height.
