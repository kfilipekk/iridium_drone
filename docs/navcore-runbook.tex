...
micro-USB power lead for the SAWbird+ (5 V from P41/P46) & 1 & 2 & AliExpress \\
...
Total, everything to buy: about GBP 757. Generated from \file{docs/PARTS.csv} by \file{tools/gen\_doc\_tables.py}; every row's reasoning and link is in the CSV, and \file{docs/BUYING.md} is the same list with prices and provenance tags.
% END GENERATED ORDER
...
Sort the bench items into two kinds, because only one kind can reach your wallet. Six of them
cannot: \file{BATT\_AMP\_PERVLT}, the IMU orientation, \file{FLOW\_ORIENT\_YAW}, flow
quality over grass, and both thermal readings are numbers you write down or calibrate, and no
possible answer changes what you should have bought. \file{U8}'s junction is worse than those
--- if it really is 139\degC{} the remedy is a \textbf{respin} --- but it is still not a
purchase.

Three could have. Each is now bought around:

\begin{center}
\begin{tabular}{@{}P{0.26\textwidth}P{0.62\textwidth}@{}}
\toprule
\textbf{Measurement} & \textbf{Why it can no longer send you shopping} \\
\midrule
\file{J2}'s ESC cable pinout & a wrong cable is re-pinnable with a fine pick \emph{and} a
spare \textbf{8-pin} JST-SH lead is on the order --- previously the only route was to rework
the cable you already had \\
the RF chain can hear Iridium & the order names a \textbf{bias-tee} dongle (Blog V3/V4)
rather than ``an RTL-SDR'', so the bench chain is one cable; and it names the SMA-to-U.FL
pigtail and the SAWbird+'s power lead, without which the chain reaches the bench but never
the aircraft \\
the grommet flange & the flange is a property of the frame kit's grommets, so a wide one meant
a different grommet. A spare silicone set is now a line item: measure, and swap rather than
order \\
\bottomrule
\end{tabular}
\end{center}

\flag{The honest residue:} one answer can still cost real money, and it is not a part. If
\file{U8} is genuinely at 139\degC{} the remedy is a new board --- which is why \textbf{three
spare bare PCBs} are on the order and why T3a is a stop gate rather than a note. \file{tools/fasteners.py} prints this list itself under
...
inventing a number \\
SGP4 and real TLE ephemeris & \flag{not built} --- the solver takes satellite state as
\emph{input}, and a circular propagator stands in for the test. That is enough to prove the
geometry inverts; it is not an ephemeris \\
Burst detection and frequency estimation & \flag{not built} --- from I/Q, to the precision the
solver needs. No RF bench run substitutes for writing it \\
The C firmware for the H743 & \flag{not built} \\
Getting the fix into the EKF & \flag{not built} --- an \file{AP\_GPS} backend or
\file{AP\_ExternalAHRS}. \file{GPS\_INPUT} is the documented route for an \emph{external}
computer, and this aircraft solves on board \\
\bottomrule
\end{tabular}
\end{center}
